...
class BinomDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.convert = transforms.Grayscale()
    ...
    def loader(self, path : Union[str, Path], gray = False) -> Image.Image:
    ...
    def __getitem__(self, idx):
        ...
        gt = self.convert(img)
        gt = torch.from_numpy(np.array(gt)).to(torch.float)
        ...
        out = torch.cat((img, gt, imgNoise),dim = 0)
