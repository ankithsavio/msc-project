...
class Trainer:
    ...
    def __init__(self, root_dir, gradient_clip_val, epochs, dataloader, model: nn.Module, lr = 1e-4, seed = 42, channels = 1, early_stop = 1000):
        ...
        self.checkpoint_manager = ocp.CheckpointManager(self.root_dir, orbax_checkpointer, options)
    ...
    def train_epoch(self, train_loader, epoch):
        ...
        for i, batch in enumerate(tqdm(train_loader, desc='Training', leave=False)):
            ...
            self.logger.add_scalar('Loss/train_per_step ', loss.item(), global_step=(epoch - 1)*self.steps_per_epoch + i)
            avg_loss += loss
